...
void initMaterials ()
{
    ...
    gmat->background_color = vec4i(55, 255, 255, 155);
...
//	lodpage->kdtree = "misc_background0.idx";
	...
	lod->addChild ( lodpage3 );

	lod->scales.push_back ( 0.2 );
// 	lod->scales.push_back ( 0.24 );
// 	lod->scales.push_back ( 0.4 );

	{
	    SLCRectNode* rc1 = new SLCRectNode ( gmat );
	    nodes.push_back ( rc1 );
	    rc1->pnts[0] = vec2f ( 0,  4 );
	    rc1->pnts[1] = vec2f ( 10, 4 );
	    rc1->pnts[2] = vec2f ( 10, 5 );
...

	{
	    SLCRectNode* rc1 = new SLCRectNode ( gmat );
	    nodes.push_back ( rc1 );
	    rc1->pnts[0] = vec2f ( 4, 0 );
	    rc1->pnts[1] = vec2f ( 5, 0 );
	    rc1->pnts[2] = vec2f ( 5, 10 );
	    rc1->pnts[3] = vec2f ( 4, 10 );
	    rc1->z = 1;
	    lodpage2->addChild ( rc1 );
	}
