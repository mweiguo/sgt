...
class BuildKdTreeTestCase1 : public ::testing::Test
{
protected:
    BuildKdTreeTestCase1 ()
    {
	// build scene data
	list<SLCNode*> nodes;
...
TEST_F ( BuildKdTreeTestCase1, getMinMax ) {
...
TEST_F ( BuildKdTreeTestCase1, computeDivision ) {
    // prepare data
    generate_n ( back_inserter(_buildkdt->_objindices), _buildkdt->_kdtree->_primitives.size(), Incr() );
    _buildkdt->_kdtree->_nodes.reserve ( _buildkdt->_kdtree->_primitives.size() * 5 );
    float minmax[4];
...
TEST_F ( BuildKdTreeTestCase1, divide ) {
    // prepare data
    generate_n ( back_inserter(_buildkdt->_objindices), _buildkdt->_kdtree->_primitives.size(), Incr() );
    _buildkdt->_kdtree->_nodes.reserve ( _buildkdt->_kdtree->_primitives.size() * 5 );
    float minmax[4];
    ...
    EXPECT_EQ ( 0.5f, _buildkdt->_objcenters[0].x() );
    EXPECT_EQ ( 0.5f, _buildkdt->_objcenters[0].y() );
    EXPECT_EQ ( 1.5f, _buildkdt->_objcenters[1].x() );
    EXPECT_EQ ( 0.5f, _buildkdt->_objcenters[1].y() );
    EXPECT_EQ ( 2.5f, _buildkdt->_objcenters[2].x() );
    EXPECT_EQ ( 0.5f, _buildkdt->_objcenters[2].y() );
    EXPECT_EQ ( 3.5f, _buildkdt->_objcenters[3].x() );
    EXPECT_EQ ( 0.5f, _buildkdt->_objcenters[3].y() );
    // should check _buildkdt->_objindices
    EXPECT_EQ ( 4u, _buildkdt->_objindices.size() );
    EXPECT_EQ ( 0, _buildkdt->_objindices[0] );
    EXPECT_EQ ( 1, _buildkdt->_objindices[1] );
    EXPECT_EQ ( 2, _buildkdt->_objindices[2] );
    EXPECT_EQ ( 3, _buildkdt->_objindices[3] );
...
TEST_F ( BuildKdTreeTestCase1, build ) {
    _buildkdt->build ();
    // should check _buildkdt->_primitives
    EXPECT_EQ ( 4u, _buildkdt->_kdtree->_primitives.size() );
    EXPECT_EQ ( 6, _buildkdt->_kdtree->_primitives[0] );
    EXPECT_EQ ( 7, _buildkdt->_kdtree->_primitives[1] );
    EXPECT_EQ ( 8, _buildkdt->_kdtree->_primitives[2] );
    EXPECT_EQ ( 9, _buildkdt->_kdtree->_primitives[3] );
}
