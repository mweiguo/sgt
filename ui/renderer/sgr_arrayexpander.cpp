...
namespace SGR
{
    ...
    void ArrayExpander::apply ( CircleNode& node )
    {
	int id = SeedGenerator::getInst().minseed();
	CircleNode* circlenode = NodeMgr::getInst().addNode<CircleNode>(id, node );

	circlenode->clear();
	_curparent->addChild ( circlenode );

	SGNode* oldparent = _curparent;
	_curparent = circlenode;
	for ( SGNode::iterator pp=node.begin(); pp!=node.end(); ++pp )
	    (*pp)->accept ( *this );
	_curparent = oldparent;
    }

    void ArrayExpander::apply ( ImageNode& node )
    {
	int id = SeedGenerator::getInst().minseed();
	ImageNode* imagenode = NodeMgr::getInst().addNode<ImageNode>(id, node );

	imagenode->clear();
	_curparent->addChild ( imagenode );

	SGNode* oldparent = _curparent;
	_curparent = imagenode;
	for ( SGNode::iterator pp=node.begin(); pp!=node.end(); ++pp )
	    (*pp)->accept ( *this );
	_curparent = oldparent;
    }

    void ArrayExpander::apply ( ImposterNode& node )
    {
	int id = SeedGenerator::getInst().minseed();
	ImposterNode* imposternode = NodeMgr::getInst().addNode<ImposterNode>(id, node );

	imposternode->clear();
	_curparent->addChild ( imposternode );

	SGNode* oldparent = _curparent;
	_curparent = imposternode;
	for ( SGNode::iterator pp=node.begin(); pp!=node.end(); ++pp )
	    (*pp)->accept ( *this );
	_curparent = oldparent;
    }
