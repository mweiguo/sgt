...
template < class T >
inline typename vec4<T>::reference vec4<T>::normalize () 
{ 
    typename vec4<T>::value_type m = mod (); 
    if ( m != 0 ) {
        _v[0] /= m; 
        _v[1] /= m; 
        _v[2] /= m; 
...
template < class T >
inline vec4<T> vec4<T>::mod ( T length ) 
{ 
    normalize();
    *this *= length;
    return *this;
...
template < class T >
inline typename vec4<T>::reference vec4<T>::operator *= ( T rhs )
{
    _v[0] *= rhs;
    _v[1] *= rhs;
    _v[2] *= rhs;
